...
{
    public class XmlDocumentFactoryConfigStandard : IFactoryConfig
    {
        ...
        string IFactoryConfig.ServiceConfigPath
        {
            get
            {
                string value;
#if DEBUG
                //DiagnosticsTool.MyDebug(string.Format("string IFactoryConfig.ServiceConfigPath = {0}", Factory.ConfigurationBuilder.Properties["ServiceKey"]));
#endif
                value = (string)Factory.ConfigurationBuilder.Properties["ServiceKey"];

                return value.IsNullOrEmpty() ? (string)Client.GetAttribute("Settings.Default.ServiceKey") : value;

                //return (string)Factory.ConfigurationBuilder.Properties["ServiceKey"];
            }
        }

        public XmlDocumentFactoryConfigStandard()
        {
#if DEBUG
